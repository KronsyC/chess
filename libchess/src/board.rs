...
impl ChessBoard {
    ...
    pub fn verify(&self){
        ...
        assert_eq!(self.whites | self.blacks, self.knights | self.queens | self.rooks | self.pawns | self.bishops | self.kings);
        assert_eq!(self.whites ^ self.blacks, self.knights ^ self.queens ^ self.rooks ^ self.pawns ^ self.bishops ^ self.kings);
    }
    ...
    pub fn team_pieces<T : TTeam>(&self) -> Bitboard {
        match T::TEAM {
            Team::Black => self.blacks,
            Team::White => self.whites,
        }
    ...
    pub fn get_piece_info(&self, piece : Position) -> Option<PieceInfo>{
        ...
        let kind = if f(self.bishops) {PieceKind::Bishop} 
                   ...
                   else if f(self.queens) {PieceKind::Queen}
                   else {return None;};
    ...
    fn is_valid_postmove<T : TTeam>(&self) -> bool{
        ...
        return !self.is_pos_attacked::<T::Enemy>(king_idx);
    }
}
